...
class Vocab(object):
    def __init__(self, vec_path=None, dim=100, fileformat='bin',voc=None, word2id=None, word_vecs=None, unk_mapping_path=None, parser=None): 
        ...
        if unk_mapping_path is not None:
            ...
            in_file.close()
        #if(parser): self.word_dim += parser.typesize
...
#         self.vocab_size, self.word_dim = map(int, header.split())
        ...
        self.vocab_size = len(self.word2id)
        self.word_vecs = np.zeros((self.vocab_size+1, self.word_dim), dtype=np.float32) # the last dimension is all zero
        #self.word_vecs = np.zeros((self.vocab_size+1, self.word_dim + self.parser.typesize), dtype=np.float32) # the last dimension is all zero
        print ("vocabulary size: ", self.vocab_size)
        print ("word dim: ", self.word_dim)
        for cur_index in xrange(self.vocab_size):
            try:
                ...
                print ("word dim from file: ", word_vecs[cur_index].shape)
